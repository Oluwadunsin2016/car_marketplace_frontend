  ...
  return (
    <div className="px-6 md:px-20 my-10">
      <h2 className="font-bold text-2xl md:text-4xl">
        {mode == "edit" ? "Update Listing" : "Add New Listing"}
      </h2>

      <form
        action=""
        onSubmit={mode == "edit" ? handleUpdate : handleSubmit}
        className="p-6 md:p-10 mt-6 border rounded-xl"
      >
        {/* Car Details */}
        <div>
            ...
            <div key={i} className="flex gap-2 items-center">
              <Checkbox
                onCheckedChange={(val: boolean) =>
                  handleFeatureChange(feature.name, val)
                }
                checked={featureData?.[feature.name]}
                className="rounded"
              />
              <h2>{feature?.label}</h2>
            </div>
        ...
        <UploadImages
          ...
          carImages={carImages}
          deleteOldImageFromFirebase={deleteOldImageFromFirebase}
        />
        <div className="mt-10 flex justify-end">
          {mode == "edit" ? (
            ...
            >
              {isLoading ? (
                <p className="flex items-center justify-center gap-1">
                  <LuLoader2 className="animate-spin ttext-lg" /> Updating
                </p>
              ) : (
                "Update"
              )}
            ...
            >
              {isLoading ? (
                <p className="flex items-center justify-center gap-1">
                  <LuLoader2 className="animate-spin ttext-lg" /> Submitting
                </p>
              ) : (
                "Submit"
              )}
